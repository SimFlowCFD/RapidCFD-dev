...
tmp<scalargpuField> mutLowReWallFunctionFvPatchScalarField::calcMut() const
{
    return tmp<scalargpuField>(new scalargpuField(patch().size(), 0.0));
}
...
tmp<scalargpuField> mutLowReWallFunctionFvPatchScalarField::yPlus() const
{
    const label patchi = patch().index();
    const turbulenceModel& turbModel =
        db().lookupObject<turbulenceModel>("turbulenceModel");
    const scalargpuField& y = turbModel.y()[patchi];
    const scalargpuField nuw
    (

...
) const
{
    if (uTheta_ < SMALL)
    {
        return tmp<scalargpuField>(new scalargpuField(size(), theta0_));
    }

    const vectorgpuField nf(patch().nf());

    // Calculated the component of the velocity parallel to the wall
    vectorgpuField Uwall(Up.patchInternalField() - Up);
    Uwall -= (nf & Uwall)*nf;

    // Find the direction of the interface parallel to the wall
    vectorgpuField nWall(nHat - (nf & nHat)*nf);
    ...
    // the interface
    scalargpuField uwall(nWall & Uwall);

    ...
    comm_(comm)
{
    ...
    lowerAddrHost_.setSize(faceOffsets.last(), -1);
    upperAddrHost_.setSize(lowerAddrHost_.size(), -1);
    ...
    for (label procMeshI = 0; procMeshI < nMeshes; procMeshI++)
    {
        ...
        const labelUList& l = procMesh.lduAddr().lowerAddrHost();
        const labelUList& u = procMesh.lduAddr().upperAddrHost();
        ...
        forAll(l, faceI)
        {
            lowerAddrHost_[allFaceI] = cellOffsets[procMeshI]+l[faceI];
            upperAddrHost_[allFaceI] = cellOffsets[procMeshI]+u[faceI];
            allFaceI++;
        }
                    ...
                    if (procMeshI < nbrProcMeshI)
                    {
                        ...
                        if (fnd != mergedMap.end())
                        {
                            ...
                            const labelUList& nbrFaceCells =
                                nbrInterfaces[nbrIntI].faceCellsHost();
                            ...
                            {
                                lowerAddrHost_[allFaceI] =
                                    cellOffsets[procMeshI]+faceCells[pfI];
                                bfMap[pfI] = allFaceI;
                                upperAddrHost_[allFaceI] =
    ...
    // Sort upper-tri order
    {
        ...
        inplaceReorder(oldToNew, lowerAddrHost_);
        inplaceReorder(oldToNew, upperAddrHost_);
    ...
    forAllConstIter(EdgeMap<labelPairList>, unmergedMap, iter)
    {
        ...
        forAll(order, i)
        {
            ...
            const labelUList& l = interfaces[interfaceI].faceCellsHost();
    ...
    if (debug)
    {
        checkUpperTriangular(cellOffsets.last(), lowerAddrHost_, upperAddrHost_);
    }
}

...
struct nutkRoughWallCalcNutFunctor
{
    const scalar Cmu25;
    const scalar kappa;
    const scalar E;

    nutkRoughWallCalcNutFunctor(const scalar Cmu25_,const scalar kappa_,const scalar E_):
...
tmp<scalargpuField> nutkRoughWallFunctionFvPatchScalarField::calcNut() const
{
    ...
    (
        ...
        nutw.begin(),
        nutkRoughWallCalcNutFunctor(Cmu25,kappa_,E_)
    );

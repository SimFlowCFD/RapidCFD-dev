    ...
    for (label levelI = 0; levelI <= agglom.size(); levelI++)
    {
        ...
        )
        {
            const labelList& cellRestrict =
                agglom.restrictAddressingHost(levelI);
            const labelList& faceRestrict =
                agglom.faceRestrictAddressingHost(levelI);
            ...
            const labelListList& patchFaceRestrict =
                agglom.patchFaceRestrictAddressingHost(levelI);
...
)
{
    ...
    const labelUList& nbr = addr.upperAddrHost();
    const labelUList& own = addr.lowerAddrHost();

    {
        ...
        forAll(interfaces, inti)
        {
            if (interfaces.set(inti))
            {
                const labelUList& faceCells = interfaces[inti].faceCellsHost();

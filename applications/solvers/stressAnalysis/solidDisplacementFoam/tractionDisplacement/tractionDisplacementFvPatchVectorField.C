...
void tractionDisplacementFvPatchVectorField::updateCoeffs()
{
    ...
    scalargpuField E(rhoE/rho);
    scalargpuField mu(E/(2.0*(1.0 + nu)));
    scalargpuField lambda(nu*E/((1.0 + nu)*(1.0 - 2.0*nu)));
    scalargpuField threeK(E/(1.0 - 2.0*nu));
    ...
    scalargpuField twoMuLambda(2*mu + lambda);

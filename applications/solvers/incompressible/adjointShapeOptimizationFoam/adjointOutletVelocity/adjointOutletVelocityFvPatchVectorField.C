...
void Foam::adjointOutletVelocityFvPatchVectorField::updateCoeffs()
{
    ...
    scalargpuField Un(mag(patch().nf() & Up));
    vectorgpuField UtHat((Up - patch().nf()*Un)/(Un + SMALL));

    vectorgpuField Uan(patch().nf()*(patch().nf() & patchInternalField()));

    vectorgpuField::operator=(phiap*patch().Sf()/sqr(patch().magSf()) + UtHat);
    //vectorgpuField::operator=(Uan + UtHat);

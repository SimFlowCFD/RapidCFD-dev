...
struct nutUSpaldingCalcUTauFunctor
{
    const scalar kappa;
    const scalar E;

    nutUSpaldingCalcUTauFunctor(const scalar kappa_,const scalar E_):kappa(kappa_),E(E_){}

    __HOST____DEVICE__
    scalar operator () (const scalar& y, const thrust::tuple<scalar,scalar,scalar,scalar>& t)
    {
        scalar nuw = thrust::get<0>(t);
        scalar nutw = thrust::get<1>(t);
        scalar magGradU = thrust::get<2>(t);
        scalar magUp = thrust::get<3>(t);
		
        scalar ut = sqrt((nutw + nuw)*magGradU);
...
tmp<scalargpuField> nutUSpaldingWallFunctionFvPatchScalarField::calcNut() const
{
...
tmp<scalargpuField> nutUSpaldingWallFunctionFvPatchScalarField::calcUTau
(
...
) const
{
    ...
    const scalargpuField& nutw = *this;
    ...
    (
        ...
        uTau.begin(),
        nutUSpaldingCalcUTauFunctor(kappa_,E_)
    );
...
tmp<scalargpuField> nutUSpaldingWallFunctionFvPatchScalarField::yPlus() const
{
    const label patchi = patch().index();

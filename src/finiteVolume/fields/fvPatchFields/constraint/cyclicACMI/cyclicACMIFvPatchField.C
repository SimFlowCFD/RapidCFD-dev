...
template<class Type>
Foam::tmp<Foam::gpuField<Type> > Foam::cyclicACMIFvPatchField<Type>::snGrad
(
...
void Foam::cyclicACMIFvPatchField<Type>::updateCoeffs()
{
    ...
    const scalargpuField& mask = cyclicACMIPatch_.cyclicACMIPatch().mask();
...
)
{
    ...
    const gpuField<Type>& cpf = *this;

    const scalargpuField& mask = cyclicACMIPatch_.cyclicACMIPatch().mask();
    gpuField<Type>::operator=(mask*cpf + npf);

...
void Foam::wallDistData<TransferType>::correct()
{
    ...
    {
        patchData.set(patchI, field_.boundaryField()[patchI].asField().ptr());
    }
    ...
    getField() = wave.distance();
//    transfer(wave.distance());

//    field_.transfer(wave.cellData());
    field_.getField() = wave.cellData();
    ...
    forAll(boundaryField(), patchI)
    {
        ...
        if (!isA<emptyFvPatchScalarField>(boundaryField()[patchI]))
        {
            ...
            boundaryField()[patchI].operator=(waveFld);

            gpuField<Type> wavePatchData(wave.patchData()[patchI].size());
            wavePatchData = wave.patchData()[patchI];

...
Foam::tmp<Foam::pointgpuField> Foam::sixDoFRigidBodyMotion::transform
(
...
struct sixDoFRigidScaleTransorm
{
    const septernion I;
    const vector zero;
    const septernion s;
    ...
    __HOST____DEVICE__
    vector operator()(const scalar& scale, const vector& initialPoint)
    {
        ...
        if (scale > SMALL)
        {
            ...
            if (scale > 1 - SMALL)
            {
                return CoR  + (Q & initQ.T() & (initialPoint - initCoR));
            }
            // Slerp septernion interpolation
            else
            {
                septernion ss(slerp(I, s, scale));

                return initCoR + ss.transform ( initialPoint );
            }
        }
        else
        {
...
) const
{
    ...
    tmp<pointgpuField> tpoints(new pointgpuField(initialPoints));
    pointgpuField& points = tpoints();

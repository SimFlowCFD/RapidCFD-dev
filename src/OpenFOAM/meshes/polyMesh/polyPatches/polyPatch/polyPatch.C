...
void Foam::polyPatch::movePoints(PstreamBuffers&, const pointgpuField& p)
{
...
const Foam::vectorgpuField Foam::polyPatch::getFaceCentres() const
{
    return patchSlice(boundaryMesh().mesh().getFaceCentres());
}
...
const Foam::vectorgpuField Foam::polyPatch::getFaceAreas() const
{
    return patchSlice(boundaryMesh().mesh().getFaceAreas());
}

...
(
    gpuField<Type>& rtf,
    const tensorgpuField& trf,
    const gpuField<Type>& tf
)
{
    thrust::transform(trf.begin(),trf.end(),tf.begin(),rtf.begin(),
                transformBinaryFunctionFunctor<tensor,Type,Type>());
}


template<class Type>
tmp<gpuField<Type> > transform
(
    const tensorgpuField& trf,
    const gpuField<Type>& tf
)
{
    tmp<gpuField<Type> > tranf(new gpuField<Type> (tf.size()));
    transform(tranf(), trf, tf);
    return tranf;
}
...
(
    const tensorgpuField& trf,
    const tmp<gpuField<Type> >& ttf
)
{
    tmp<gpuField<Type> > tranf = reuseTmp<Type, Type>::New(ttf);
    transform(tranf(), trf, ttf());
    reuseTmp<Type, Type>::clear(ttf);
    return tranf;
}
...
template<class Type1, class Type2>
tmp<gpuField<Type1> > transformFieldMask(const gpuField<Type2>& f)
{
    return f;
}

template<class Type1, class Type2>
tmp<gpuField<Type1> > transformFieldMask(const tmp<gpuField<Type2> >& tf)
{
    return tmp<gpuField<Type1> >(tf.ptr());
}

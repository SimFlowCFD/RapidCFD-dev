...
void Foam::porousBafflePressureFvPatchField<Foam::scalar>::updateCoeffs()
{
    ...
    scalargpuField Un(phip/patch().magSf());

    scalargpuField magUn(mag(Un));
    ...
    if (phi.dimensions() == dimensionSet(0, 3, -1, 0, 0))
    {
        ...
        const scalargpuField nu = turbModel.nu()().boundaryField()[patchI];
    ...
    else
    {
        ...
        const scalargpuField mu = turbModel.mu().boundaryField()[patchI];

        const scalargpuField rhow =

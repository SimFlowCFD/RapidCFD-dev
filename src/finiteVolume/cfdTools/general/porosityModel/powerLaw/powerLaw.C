...
) const
{
    ...
    apply(Udiag, V, rho.getField(), U.getField());

    force = Udiag*U.getField();
}
...
) const
{
    ...
    if (UEqn.dimensions() == dimForce)
    {
        ...
        apply(AU.getField(), rho.getField(), U);
    }
    else
    {
        apply(AU.getField(), geometricOneField(), U);
    }
}

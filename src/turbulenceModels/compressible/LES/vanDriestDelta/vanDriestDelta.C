...
void vanDriestDelta::calcDelta()
{
    ...
    forAll(patches, patchi)
    {
        if (isA<wallFvPatch>(patches[patchi]))
        {
            ...
            const scalargpuField& rhow = rho.boundaryField()[patchi];
            const scalargpuField& muw = mu.boundaryField()[patchi];
            const scalargpuField& muSgsw = muSgs().boundaryField()[patchi];

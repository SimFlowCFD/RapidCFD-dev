...
namespace Foam
{
struct pressureGradientExplicitSourceMagUbarAveFunctor{
	const vector flowDir;
	pressureGradientExplicitSourceMagUbarAveFunctor(const vector _flowDir):flowDir(_flowDir){}
	__HOST____DEVICE__
	vector operator ()(const vector& U, const scalar& V){
		return (flowDir & U)*V;
	}
};
...
{
    const scalargpuField& rAU = invAPtr_().internalField();

    // Integrate flow variables over cell set
    scalar magUbarAve = 0.0;
    scalar rAUave = 0.0;
    const scalargpuField& cv = mesh_.V();
    forAll(cells_, i)
    {
...
)
{
    ...
    gpuIndirectList<vector>(Su.getField(), cells_) = flowDir_*gradP;

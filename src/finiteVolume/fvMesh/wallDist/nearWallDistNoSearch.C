...
void Foam::nearWallDistNoSearch::doAll()
{
    ...
    forAll(patches, patchI)
    {
        ...
        {
            const labelgpuList& faceCells = patches[patchI].faceCells();
...
*/
            thrust::transform(thrust::make_zip_iterator(thrust::make_tuple(
                                                                       Apatch.begin(),
                                                                       patchCentres.begin(),
                                                                       thrust::make_permutation_iterator(cellCentres.getField().begin(),faceCells.begin()),
                                                                       magApatch.begin())),
                              thrust::make_zip_iterator(thrust::make_tuple(
                                                                       Apatch.end(),
                                                                       patchCentres.end(),
                                                                       thrust::make_permutation_iterator(cellCentres.getField().begin(),faceCells.end()),
                                                                       magApatch.end())),
                              ypatch.begin(),
                              nearWallDistFunctor());
        }

...
struct atmBoundaryLayerInletEpsilonUstarFunctor{
	const scalar kappa_;
	const scalar Uref_;
	const scalar Href_;
	atmBoundaryLayerInletEpsilonUstarFunctor(scalar kappa,scalar Uref, scalar Href):
	kappa_(kappa),
	Uref_(Uref),
	Href_(Href)
	{}
	__HOST____DEVICE__
	scalar operator () (const scalar& z0){
		return kappa_*Uref_/(log((Href_  + z0)/max(z0 , 0.001)));
	}
};
    ...
    Ustar_(p.size())
{
    ...
    thrust::transform(z0_.begin(),z0_.end(),Ustar_.begin(),
                      atmBoundaryLayerInletEpsilonUstarFunctor(kappa_,Uref_,Href_));
    ...
    */
    ...
    scalargpuField::operator=(pow3(Ustar_)/(kappa_*((c & z_) - zGround_ + z0_)));
}

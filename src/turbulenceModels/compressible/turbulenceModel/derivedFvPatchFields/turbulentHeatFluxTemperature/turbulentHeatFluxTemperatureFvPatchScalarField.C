    ...
    QrName_(dict.lookupOrDefault<word>("Qr", "none"))
{
    ...
    {
        fvPatchField<scalar>::operator=(gpuField<scalar>("value", dict, p.size()));
        gradient() = gpuField<scalar>("gradient", dict, p.size());
    }
    else
    {
...
void turbulentHeatFluxTemperatureFvPatchScalarField::updateCoeffs()
{
    ...
    const scalargpuField& Tp = *this;

    scalargpuField qr(this->size(), 0.0);
    ...
    switch (heatSource_)
    {
        case hsPower:
        {
            const scalar Ap = sum(patch().magSf());

...
Foam::directionMixedFvPatchField<Type>::snGrad() const
{
    ...
    tmp<gpuField<Type> > normalValue = transform(valueFraction_, refValue_);

    tmp<gpuField<Type> > gradValue = pif + refGrad_/this->patch().deltaCoeffs();

    tmp<gpuField<Type> > transformGradValue =
...
void Foam::directionMixedFvPatchField<Type>::evaluate(const Pstream::commsTypes)
{
    ...
    tmp<gpuField<Type> > gradValue =
        this->patchInternalField() + refGrad_/this->patch().deltaCoeffs();

    tmp<gpuField<Type> > transformGradValue =
        transform(I - valueFraction_, gradValue);

    gpuField<Type>::operator=(normalValue + transformGradValue);
...
{
    vectorgpuField diag(valueFraction_.size());

...
:
    ...
    radiationCoupledBase(p, "undefined", scalargpuField::null()),
    TName_("T")
{
    refValue() = 0.0;
    refGrad() = 0.0;
...
void Foam::MarshakRadiationFvPatchScalarField::updateCoeffs()
{
    ...
    const scalargpuField& gamma =
        patch().lookupPatchField<volScalarField, scalar>("gammaRad");

    const scalargpuField temissivity = emissivity();

    const scalargpuField Ep(temissivity/(2.0*(2.0 - temissivity)));

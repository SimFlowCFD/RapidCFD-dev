...
struct alphatJayatillekeFunctor
{
    const scalar Cmu25;
    const scalar Prt;
    const scalar kappa;
    ...
    alphatJayatillekeFunctor(scalar Cmu25_,scalar Prt_,scalar kappa_,scalar E_,scalar tolerance_,int maxIters_):
                             Cmu25(Cmu25_),Prt(Prt_),kappa(kappa_),E(E_),tolerance(tolerance_),maxIters(maxIters_){}
    __HOST____DEVICE__
    scalar operator()(const scalar& y, const thrust::tuple<scalar,scalar,scalar,scalar,scalar,scalar,vector>& t)
    {
        scalar k = thrust::get<0>(t);
        scalar muw = thrust::get<1>(t);
        scalar rhow = thrust::get<2>(t);
        scalar alphaw = thrust::get<3>(t);
        scalar magUp = thrust::get<4>(t);
        scalar qDot = thrust::get<5>(t);
        vector Uw = thrust::get<6>(t);
        ...
        scalar _yPlusTherm = yPlusTherm(P, Prat,kappa,E,tolerance,maxIters);

        // Evaluate new effective thermal diffusivity
        scalar alphaEff = 0.0;
        if (yPlus < _yPlusTherm)
        {
            scalar A = qDot*rhow*uTau*y;
            scalar B = qDot*Pr*yPlus;
            scalar C = Pr*0.5*rhow*uTau*sqr(magUp);
            alphaEff = A/(B + C + VSMALL);
        }
        else
        {
            scalar A = qDot*rhow*uTau*y;
            scalar B = qDot*Prt*(1.0/kappa*log(E*yPlus) + P);
            scalar magUc = uTau/kappa*log(E*_yPlusTherm) - mag(Uw);
            scalar C =
                0.5*rhow*uTau
               *(Prt*sqr(magUp) + (Pr - Prt)*sqr(magUc));
            alphaEff = A/(B + C + VSMALL);
        }

        // Update turbulent thermal diffusivity
        return max(0.0, alphaEff - alphaw);
     }
};
...
void alphatJayatillekeWallFunctionFvPatchScalarField::updateCoeffs()
{
    ...
    const scalargpuField& alphaw = turbModel.alpha().boundaryField()[patchi];
    scalargpuField& alphatw = *this;
    ...
    (
        ...
        alphatw.begin(),
        alphatJayatillekeFunctor(Cmu25,Prt_,kappa_,E_,tolerance_,maxIters_)
    );

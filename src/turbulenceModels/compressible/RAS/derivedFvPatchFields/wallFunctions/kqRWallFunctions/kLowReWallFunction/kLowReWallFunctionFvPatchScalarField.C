...
struct kLowReWallFunctionFunctor{
	const scalar yPlusLam_;
	const scalar Cmu25_;
	const scalar Ceps2_;
	const scalar kappa_;
	kLowReWallFunctionFunctor(const scalar yPlusLam,
	                          const scalar Cmu25,
	                          const scalar Ceps2,
	                          const scalar kappa):
	yPlusLam_(yPlusLam),
	...
	scalar operator ()(const thrust::tuple<scalar,scalar,scalar,scalar>& t){
        ...
        if (yPlus > yPlusLam_)
        {
            scalar Ck = -0.416;
            scalar Bk = 8.366;
            return (Ck/kappa_*log(yPlus) + Bk)*sqr(uTau);
        }
        else
        {
            scalar C = 11.0;
            scalar Cf = (1.0/sqr(yPlus + C) + 2.0*yPlus/pow3(C) - 1.0/sqr(C));
            return (2400.0/sqr(Ceps2_)*Cf)*sqr(uTau);
        }
...
void kLowReWallFunctionFvPatchScalarField::updateCoeffs()
{
    ...
    scalargpuField& kw = *this;
    
    thrust::transform(thrust::make_zip_iterator(thrust::make_tuple(thrust::make_permutation_iterator(k.getField().begin(),patch().faceCells().begin()),y.begin(),muw.begin(),rhow.begin())),
                      thrust::make_zip_iterator(thrust::make_tuple(thrust::make_permutation_iterator(k.getField().begin(),patch().faceCells().end()),y.end(),muw.end(),rhow.end())),
                      kw.begin(),
                      kLowReWallFunctionFunctor(yPlusLam_,Cmu25,Ceps2_,kappa_));
/*
    // Set k wall values
    forAll(kw, faceI)
    {

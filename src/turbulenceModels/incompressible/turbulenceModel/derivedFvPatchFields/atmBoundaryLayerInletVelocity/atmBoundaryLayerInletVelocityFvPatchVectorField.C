...
struct atmBoundaryLayerInletVelocityFvPatchVectorFieldUstarFunctor{
	const scalar kappa_;
    const scalar Uref_;
    const scalar Href_;
    atmBoundaryLayerInletVelocityFvPatchVectorFieldUstarFunctor(const scalar kappa, const scalar Uref, const scalar Href):
       kappa_(kappa),
       Uref_(Uref),
       Href_(Href)
    {}
...
struct atmBoundaryLayerInletVelocityFvPatchVectorFieldUnFunctor{
	const scalar kappa_;
    const scalar Uref_;
    const scalar Href_;
    atmBoundaryLayerInletVelocityFvPatchVectorFieldUnFunctor(const scalar kappa, const scalar Uref, const scalar Href):
       kappa_(kappa),
       Uref_(Uref),
       Href_(Href)
    {}
	...
	scalar operator () (const thrust::tuple<scalar,scalar,scalar,scalar>& t) {
		...
		scalar coord = thrust::get<0>(t);
		scalar Ustar_ = thrust::get<1>(t);
		scalar zGround_ = thrust::get<2>(t);
		scalar z0_ = thrust::get<3>(t);

		if ((coord - zGround_) < Href_)
        {
            return (Ustar_/kappa_)
...
*/
    thrust::transform(z0_.begin(),z0_.end(),Ustar_.begin(),
                      atmBoundaryLayerInletVelocityFvPatchVectorFieldUstarFunctor(kappa_,Uref_,Href_));
...
/*
    ...
    */
    thrust::transform(thrust::make_zip_iterator(thrust::make_tuple(coord.begin(),Ustar_.begin(),zGround_.begin(),z0_.begin())),
                      thrust::make_zip_iterator(thrust::make_tuple(coord.end(),Ustar_.end(),zGround_.end(),z0_.end())),
                     Un.begin(),
                     atmBoundaryLayerInletVelocityFvPatchVectorFieldUnFunctor(kappa_,Uref_,Href_));

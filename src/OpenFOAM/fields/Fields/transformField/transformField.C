...
Foam::tmp<Foam::vectorgpuField> Foam::transform
(
    const quaternion& q,
    const vectorgpuField& tf
)
{
    tmp<vectorgpuField > tranf(new vectorgpuField(tf.size()));
    transform(tranf(), q, tf);
    return tranf;
}
...
(
    const quaternion& q,
    const tmp<vectorgpuField>& ttf
)
{
    tmp<vectorgpuField > tranf = reuseTmp<vector, vector>::New(ttf);
    transform(tranf(), q, ttf());
    reuseTmp<vector, vector>::clear(ttf);
    return tranf;
}

...
struct motionSmootherAlgoCheckFldFunctor
{
    __HOST____DEVICE__
    bool operator()(const scalar& val)
    {
        return (val > -GREAT) && (val < GREAT);
    }
};
...
struct motionSmootherAlgoMinSmoothFunctor
{
    ...
    scalar operator()(const bool& isAffected, const Tuple& t)
    {
        ...
        const scalar& fld = thrust::get<1>(t);
        const scalar& avgFld = thrust::get<2>(t);
...
(
    const scalargpuField& edgeWeights,
    const boolgpuList& isAffectedPoint,
...
:
    ...
    gpuIsInternalPoint_(mesh_.nPoints(), true),

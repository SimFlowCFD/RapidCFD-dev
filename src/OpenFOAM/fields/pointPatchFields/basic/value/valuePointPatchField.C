    ...
    gpuField<Type>(p.size())
{
    ...
    {
        gpuField<Type>::operator=(pTraits<Type>::zero);
    }
    else
    {
        FatalIOErrorIn
...
void Foam::valuePointPatchField<Type>::write(Ostream& os) const
{
    ...
    Field<Type> tmpField(this->asField());
 
    tmpField.writeEntry("value", os);
}
...
{
    gpuField<Type>::operator=(ptf.patchInternalField());
}

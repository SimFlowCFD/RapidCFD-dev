...
struct pointPatchMapperCalcAddressingUnmapped: public std::unary_function<label,bool>
{
    __HOST____DEVICE__
    bool operator()(const label& addr)
    {
        return addr < 0;
    }
};
...
void Foam::pointPatchMapper::calcAddressing() const
{
    ...
    {
        // Direct mapping.
        directAddrPtr_ = new labelgpuList(mpm_.patchPointMap()[patch_.index()]);
        labelgpuList& addr = *directAddrPtr_;
        ...
        hasUnmapped_ = 
            ...
            (
                ...
                (
                    addr.begin(),
                    pointPatchMapperCalcAddressingUnmapped()
                ),
                thrust::make_transform_iterator
                (
                    addr.end(),
    ...
    {
        notImplemented("pointPatchMapper::calcAddressing()");
        /*
...
const Foam::labelgpuList& Foam::pointPatchMapper::directAddressing() const
{
    if (!direct())
    {
        FatalErrorIn
...
const Foam::labelgpuListList& Foam::pointPatchMapper::addressing() const
{
    if (direct())
    {
        FatalErrorIn
...
const Foam::scalargpuListList& Foam::pointPatchMapper::weights() const
{
    if (direct())
    {
        FatalErrorIn

...
struct limitedSurfaceInterpolationSchemeWeightsFunctor{
    ...
    scalar operator()(const thrust::tuple<scalar,scalar,scalar>& t){
        return thrust::get<0>(t)*thrust::get<1>(t)
          + (1.0 - thrust::get<0>(t))*pos(thrust::get<2>(t));
    }
};
...
) const
{
    ...
    scalargpuField& pWeights = Weights.internalField();
/*
...
*/
    thrust::transform(thrust::make_zip_iterator(thrust::make_tuple(pWeights.begin(),CDweights.getField().begin(),faceFlux_.getField().begin())),
                      thrust::make_zip_iterator(thrust::make_tuple(pWeights.end(),CDweights.getField().end(),faceFlux_.getField().end())),
                      pWeights.begin(),

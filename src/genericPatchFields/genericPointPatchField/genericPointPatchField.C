            ...
            )
            {
                ...
                )
                {
                    ...
                    (
                     ...
                     == token::Compound<gpuList<scalar> >::typeName
                    )
                    {
                        scalargpuField* fPtr = new scalargpuField;
                        fPtr->transfer
                        (
                            dynamicCast<token::Compound<gpuList<scalar> > >
                            (
                    ...
                    (
                        fieldToken.compoundToken().type()
                     == token::Compound<gpuList<vector> >::typeName
                    )
                    {
                        vectorgpuField* fPtr = new vectorgpuField;
                        fPtr->transfer
                        (
                            dynamicCast<token::Compound<gpuList<vector> > >
                            (
                                fieldToken.transferCompoundToken(is)
                            )
                        );
                    ...
                    (
                        fieldToken.compoundToken().type()
                     == token::Compound<gpuList<sphericalTensor> >::typeName
                    )
                    {
                        sphericalTensorgpuField* fPtr = new sphericalTensorgpuField;
                        fPtr->transfer
                        (
                            dynamicCast
                            <
                                token::Compound<gpuList<sphericalTensor> >
                            >
                            (
                                fieldToken.transferCompoundToken(is)
                            )
                    ...
                    (
                        fieldToken.compoundToken().type()
                     == token::Compound<gpuList<symmTensor> >::typeName
                    )
                    {
                        symmTensorgpuField* fPtr = new symmTensorgpuField;
                        fPtr->transfer
                        (
                            dynamicCast
                            <
                                token::Compound<gpuList<symmTensor> >
                            >
                            (
                                fieldToken.transferCompoundToken(is)
                            )
                    ...
                    (
                        fieldToken.compoundToken().type()
                     == token::Compound<gpuList<tensor> >::typeName
                    )
                    {
                        tensorgpuField* fPtr = new tensorgpuField;
                        fPtr->transfer
                        (
                            dynamicCast<token::Compound<gpuList<tensor> > >
                            (
                                fieldToken.transferCompoundToken(is)
                            )
                        );
    ...
    dict_(ptf.dict_)
{
    ...
    )
    {
        ...
        (
            iter.key(),
            new scalargpuField(*iter(), mapper)
        );
    }

    forAllConstIter
    (
        HashPtrTable<vectorgpuField>,
        ptf.vectorFields_,
        iter
    )
    {
        ...
        (
            iter.key(),
            new vectorgpuField(*iter(), mapper)
        );
    }

    forAllConstIter
    (
        HashPtrTable<sphericalTensorgpuField>,
    ...
    )
    {
        ...
        (
            iter.key(),
            new sphericalTensorgpuField(*iter(), mapper)
        );
    }

    forAllConstIter
    (
        HashPtrTable<symmTensorgpuField>,
    ...
    )
    {
        ...
        (
            iter.key(),
            new symmTensorgpuField(*iter(), mapper)
        );
    }

    forAllConstIter
    (
        HashPtrTable<tensorgpuField>,
        ptf.tensorFields_,
        iter
    )
    {
        ...
        (
            iter.key(),
            new tensorgpuField(*iter(), mapper)
        );
    }
}
...
)
{
    ...
    {
        HashPtrTable<scalargpuField>::const_iterator dptfIter =
    ...
    {
        HashPtrTable<vectorgpuField>::const_iterator dptfIter =
    ...
    {
        HashPtrTable<sphericalTensorgpuField>::const_iterator dptfIter =
    ...
    {
        HashPtrTable<symmTensorgpuField>::const_iterator dptfIter =
    ...
    {
        HashPtrTable<tensorgpuField>::const_iterator dptfIter =

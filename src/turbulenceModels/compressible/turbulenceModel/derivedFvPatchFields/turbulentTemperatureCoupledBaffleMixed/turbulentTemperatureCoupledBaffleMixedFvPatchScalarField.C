...
struct turbulentTemperatureCoupledBaffleMixedFunctor{
	__HOST____DEVICE__
	scalar operator () (const scalar& tl,const scalar& kl){
		if (tl > 0.0)
		{
			return kl/tl;
		}
    ...
    contactRes_(0.0)
{
    ...
    if (dict.found("thicknessLayers"))
    {
        ...
        {
			scalargpuField resTmp(thicknessLayers_.size());
			
			thrust::transform(thicknessLayers_.begin(),thicknessLayers_.end(),
			                 kappaLayers_.begin(),
			                 resTmp.begin(),
			                 turbulentTemperatureCoupledBaffleMixedFunctor());
			contactRes_ = thrust::reduce(resTmp.begin(),resTmp.end(),contactRes_,thrust::plus<scalar>());
			/*
...
void turbulentTemperatureCoupledBaffleMixedFvPatchScalarField::updateCoeffs()
{
    ...
    tmp<scalargpuField> nbrIntFld(new scalargpuField(nbrField.size(), 0.0));
    tmp<scalargpuField> nbrKDelta(new scalargpuField(nbrField.size(), 0.0));

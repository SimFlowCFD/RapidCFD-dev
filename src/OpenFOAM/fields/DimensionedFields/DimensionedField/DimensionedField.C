...
:
    regIOobject(io),
    mesh_(mesh),
    field_(GeoMesh::size(mesh), dt.value()),
...
) const
{
    ...
    Foam::component(result().getField(), field_, d);
...
DimensionedField<Type, GeoMesh>::T() const
{
    ...
    Foam::T(result().getField(), getField());
...
) const
{
    ...
    (
        ...
        (
            ...
            this->dimensions(),
            gSum(weightField.getField()*getField())/gSum(weightField.getField())
        )
    );
}
...
)
{
    ...
    dimensions_ = df.dimensions();
    field_.transfer(const_cast<gpuField<Type>& >(df.getField()));
    tdf.clear();
}

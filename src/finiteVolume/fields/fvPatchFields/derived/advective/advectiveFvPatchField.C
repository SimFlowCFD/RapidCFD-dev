...
void Foam::advectiveFvPatchField<Type>::updateCoeffs()
{
    ...
    const scalargpuField w(Foam::max(advectionSpeed(), scalar(0)));

    // Calculate the field wave coefficient alpha (See notes)
    const scalargpuField alpha(w*deltaT*this->patch().deltaCoeffs());
    ...
    if (lInf_ > 0)
    {
        ...
        const scalargpuField k(w*deltaT/lInf_);

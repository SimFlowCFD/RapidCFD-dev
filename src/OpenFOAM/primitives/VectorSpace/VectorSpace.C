...
)
{
    ...
    {
        is >> static_cast<Form&>(*this).operator[](i);
    }
...
)
{
    ...
    {
        buf << static_cast<const Form&>(vs).operator[](i) << ',';
    }

    buf << static_cast<const Form&>(vs).operator[](nCmpt-1) << ')';
...
)
{
    ...
    for (int i=0; i<nCmpt; i++)
    {
        is >> static_cast<Form&>(vs).operator[](i);
    }
...
)
{
    ...
    for (int i=0; i<nCmpt-1; i++)
    {
        os << static_cast<const Form&>(vs).operator[](i) << token::SPACE;
    }

    os << static_cast<const Form&>(vs).operator[](nCmpt-1) << token::END_LIST;

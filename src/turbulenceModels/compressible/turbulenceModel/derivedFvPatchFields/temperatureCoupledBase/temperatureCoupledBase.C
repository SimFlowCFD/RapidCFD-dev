...
Foam::tmp<Foam::scalargpuField> Foam::temperatureCoupledBase::kappa
(
    const scalargpuField& Tp
) const
{
    ...
    switch (method_)
    {
        ...
        case mtDirectionalSolidThermo:
        {
            ...
            const symmTensorgpuField& alphaAni =
                patch_.lookupPatchField<volSymmTensorField, scalar>
                (
                    alphaAniName_
                );

            const scalargpuField& pp = thermo.p().boundaryField()[patchI];

            const symmTensorgpuField kappa(alphaAni*thermo.Cp(pp, Tp, patchI));

            const vectorgpuField n(patch_.nf());
        ...
        case mtLookup:
        {
            ...
            {
                const symmTensorgpuField& KWall =
                    patch_.lookupPatchField<volSymmTensorField, scalar>
                    (
                        kappaName_
                    );

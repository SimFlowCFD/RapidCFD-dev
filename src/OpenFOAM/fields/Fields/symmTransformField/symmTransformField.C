...
)
{
  /*  if (trf.size() == 1)
    ...
    thrust::transform(trf.begin(),trf.end(),tf.begin(),rtf.begin(),
                transformBinaryFunctionFunctor<symmTensor,Type,Type>());
}
...
tmp<gpuField<Type> > transform
(
    const tmp<symmTensorgpuField>& ttrf,
    const gpuField<Type>& tf
)
{
    tmp<gpuField<Type> > tranf(new gpuField<Type> (tf.size()));
...
)
{
    thrust::transform(tf.begin(),tf.end(),rtf.begin(),
            transformBinaryFunctionSFFunctor<symmTensor,Type,Type>(t));
}

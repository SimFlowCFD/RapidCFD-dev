...
void Foam::fixedShearStressFvPatchVectorField::updateCoeffs()
{
    ...
    else if (phi.dimensions() == dimDensity*dimVelocity*dimArea)
    {
        ...
        nuEff = turbModel.muEff()().boundaryField()[patchI]/rhop;
    }
    else
    {
    ...
    const vectorgpuField Uc(patchInternalField());

    vector tauHat = tau0_/(mag(tau0_) + ROOTVSMALL);

    const scalargpuField& ry = patch().deltaCoeffs();

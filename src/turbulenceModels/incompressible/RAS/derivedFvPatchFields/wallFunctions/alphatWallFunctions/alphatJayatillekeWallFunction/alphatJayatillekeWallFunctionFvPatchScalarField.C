...
__HOST____DEVICE__
scalar alphatJayatillekePsmooth
(
    const scalar Prat
)
{
...
{
    return alphatJayatillekePsmooth(Prat);
}

__HOST____DEVICE__
scalar alphatJayatillekeyPlusTherm
(
    const scalar P,
    const scalar Prat,
    const scalar E_,
...
{
	return alphatJayatillekeyPlusTherm(P,Prat,E_,kappa_,tolerance_,maxIters_);
}
...
// * * * * * * * * * * * * * * * Member Functions  * * * * * * * * * * * * * //
struct alphatJayatillekeWallFunctionFunctor{
	...
	const scalar tolerance_;
	const label maxIters_;
	alphatJayatillekeWallFunctionFunctor(const scalar Cmu25,
	                          const scalar Prt,
	                          const scalar Pr,
	                          const scalar E,
	                          const scalar kappa,
	...
	scalar operator ()(const thrust::tuple<scalar,scalar,scalar>& t){
		...
		scalar yPlus = Cmu25_*sqrt(k)*y/nuw;

        // Molecular-to-turbulent Prandtl number ratio
        scalar Prat = Pr_/Prt_;

        // Thermal sublayer thickness
        scalar P = alphatJayatillekePsmooth(Prat);
        scalar yPlusTherm = alphatJayatillekeyPlusTherm(P, Prat,E_,kappa_,tolerance_,maxIters_);

        // Update turbulent thermal conductivity
        if (yPlus > yPlusTherm)
        {
            scalar nu = nuw;
            scalar kt = nu*(yPlus/(Prt_*(log(E_*yPlus)/kappa_ + P)) - 1/Pr_);
            return max(0.0, kt);
        }
        else
        {
...
void alphatJayatillekeWallFunctionFvPatchScalarField::updateCoeffs()
{
    ...
    thrust::transform(thrust::make_zip_iterator(thrust::make_tuple(thrust::make_permutation_iterator(k.getField().begin(),patch().faceCells().begin()),y.begin(),nuw.begin())),
                      thrust::make_zip_iterator(thrust::make_tuple(thrust::make_permutation_iterator(k.getField().begin(),patch().faceCells().end()),y.end(),nuw.end())),
                     alphatw.begin(),
                     alphatJayatillekeWallFunctionFunctor(Cmu25,Prt_,Pr,E_,kappa_,tolerance_,maxIters_));
    /*

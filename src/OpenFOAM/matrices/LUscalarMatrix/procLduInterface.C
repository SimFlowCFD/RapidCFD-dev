...
:
    faceCells_(interface.interface().faceCellsHost()),
    coeffs_(coeffs.size()),
    myProcNo_(-1),
    neighbProcNo_(-1),
    tag_(-1),
...
void Foam::procLduInterface::init(const lduInterfaceField& interface)
{
    ...
    else
    {
        FatalErrorIn
        (
            "procLduInterface::init(const lduInterfaceField&)"
